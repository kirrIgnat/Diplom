function [result] = gidro_new_eq(alpha,beta, Nx)
Nx1=Nx+1; Nx2=Nx/2;
xbeg=0.; xend=1.;
h=(xend-xbeg)/Nx;
...
for i=1:Nx2
    ...
    %pinf_a(i) = 8.5 *10^8;
    T(i)=p(i)/(Ra*roa(i));
    p(i)= roa(i) * (gma-1/gma) * cpa *T(i)- pinf_a;
    E(i)=(cpa/gma) * T(i) + (p(i)/roa(i));
end
%Air
for i=(Nx2+1):Nx
    ...
    %pinf_b(i) = 0;
    T(i)=p(i)/(Rb*rob(i));
    p(i)= roa(i) * (gmb-1/gmb) * cpb *T(i)- pinf_b;
    E(i)=(cpb/gmb) * T(i) + (p(i)/rob(i));
end
ro=roa+rob;
